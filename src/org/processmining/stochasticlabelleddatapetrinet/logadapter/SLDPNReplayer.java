	...
	public double replay(StochasticLabelledDataPetriNet<?> sldpn, XTrace trace) {
		...
		for (XEvent event : trace) {
			...
			if (idx != null && enabledTransitions.get(idx)) {
				...
				System.out.println("Executing transition " + name + "(" + idx + ") with weight "
						+ semantics.getTransitionWeight(idx) + " and probability "
						+ (semantics.getTransitionWeight(idx) / semantics.getTotalWeightOfEnabledTransitions()));
